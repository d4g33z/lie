...
local _index rnk;     /* Lie rank of full group */
local _index eps_dim; /* dimension of $\eps$-space */
local _index perm_size;	/* number of entries to be permuted */
...
local _index cox_order, X_order;
...
{ _index i=rnk; entry sum=0; while (e[i]=sum,--i>=0) sum+=w[i]; }

local void e2wAn(entry const* e,entry* w)
{ _index i; for (i=0; i<rnk; ++i) w[i]=e[i]-e[i+1]; }

local void w2eBn(entry const* w,entry* e)
{ _index i=rnk-1; entry sum=w[i]; while (e[i]=sum,--i>=0) sum+=2*w[i]; }

local void e2wBn(entry const* e,entry* w)
{ _index i; for (i=0; i<rnk-1; ++i) w[i]=(e[i]-e[i+1])/2; w[i]=e[i]; }

local void w2eCn(entry const* w,entry* e)
{ _index i=rnk-1; entry sum=w[i]; while (e[i]=sum,--i>=0) sum+=w[i]; }

local void e2wCn(entry const* e,entry* w)
{ _index i; for (i=0; i<rnk-1; ++i) w[i]=e[i]-e[i+1]; w[i]=e[i]; }

local void w2eDn(entry const* w,entry* e)
{ _index i=rnk-1; entry sum=w[i]-w[i-1]; while(e[i]=sum,--i>=0) sum+=2*w[i]; }

local void e2wDn(entry const* e,entry* w)
{ _index i; for (i=0; i<rnk-1; ++i) w[i]=(e[i]-e[i+1])/2; w[i]=w[i-1]+e[i]; }

local void w2eE68(entry const* w,entry* e)
{ _index i, u=perm_size;
...
{ _index i; e[0]=r[0]; for (i=1;i<rnk;++i) e[i]=r[i]-r[i-1]; e[i]=-r[i-1]; }

local void e2rAn (entry const* e,entry* r)
{ _index i; entry sum=r[0]=e[0]; for (i=1;i<rnk;++i) r[i]=sum+=e[i]; }

local void r2eBn (entry const* r,entry* e)
{ _index i; e[0]=r[0]; for (i=1;i<rnk;++i) e[i]=r[i]-r[i-1]; }
...
{ _index i; e[0]=r[0]; for (i=1;i<rnk;++i) e[i]=r[i]-r[i-1]; e[i-2]+=r[i-1]; }
...
{ freemem(temp1); freemem(temp2); freemem(suborbit_reps);
  { _index i; for (i=0; i<X_order; ++i) freemem(X_elt[i]); }
  if (cox!=NULL) freemem(cox);
}

void Weylloop(void(* action )(entry*),entry* v)
{ entry* tmp=temp1->compon;
  _index k,* inx=NULL; /* indices of non-zero entries in |w| */

  tabulate_suborbits(v);

  if (subtype!='A') inx=alloc_array(_index,perm_size+1);
  ...
  { entry* w=suborbit_reps->elm[k]; /* it is safe to modify this vector */
	...
	{ (*from_e)(w,tmp); (*action)(tmp);
	    ...
	    { _index i=0; unsigned long bits=signcount++;
