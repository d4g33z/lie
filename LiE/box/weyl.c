...
void simp_w_refl(entry* w, _index i, simpgrp* g)
{ _index r=g->lierank; entry wi=w[i];
...
local void simp_rt_refl (entry* w,_index i,simpgrp* g)
{ _index r=g->lierank; entry c= -w[i]+(i>0?w[i-1]:0)+(i<r-1?w[i+1]:0);
  switch (g->lietype)
...
void w_refl(entry* lambda, _index wi)
...
{ _index i,j,r=g->lierank; matrix* res=mkmatrix(r,r);
...
{ _index i,j,r=Lierank(grp); matrix* res=mkmatrix(r,r); entry** m=res->elm;
  for (i=0; i<r; ++i)
  { for (j=0; j<r; ++j) m[i][j]= i==j;
    Waction(m[i],word);
  }
...
{ _index i,j,r=Ssrank(grp); matrix* res=mkmatrix(r,r); entry** m=res->elm;
  for (i=0; i<r; ++i)
  { for (j=0; j<r; ++j) m[i][j]= i==j;
    Wrtaction(m[i],word);
  }
...
{ _index i,r=g->lierank; bigint* result=num;
...
{ _index i; bigint* result=copybigint(one,NULL);
...
{ _index i,j,s=Ssrank(grp), n=v->ncomp; matrix* roots=mkmatrix(n,s);
...
{ _index i,d,s=Ssrank(grp); entry* x=mkintarray(s),* y=x; bigint* result=one;
...
{ _index i,j,k,r=Lierank(grp),s=Ssrank(grp);
  matrix* result; entry** m;
  _index level_start=0, level_end=1, cur=1;
...
{ _index i,j,r=Lierank(grp),s=Ssrank(grp);
...
{ _index i,k=0,l=0,r=p->ncols; poly* result; entry** res;
  ...
  for (i=0; i<p->nrows; ++i)
  { _index j; matrix* orbit=Weyl_orbit(p->elm[i],NULL); entry** x=orbit->elm;
...
{ _index i,k=0,r=p->ncols; poly* result; entry** res,*rho=mkintarray(r);
  ...
  for (i=0; i<p->nrows; ++i)
  { _index j,l; matrix* orbit=Weyl_orbit(p->elm[i],NULL); entry** x=orbit->elm;
