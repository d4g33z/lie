...
static _index n_parts(_index n)
{ _index i,k,np; entry* c=mkintarray(n+1); /* coefficients */
...
{ _index i,j,k=0; entry* h; bigint* res=copybigint(one,NULL);
...
{ _index prev=0,i=0,j,n=0,k,f=1; bigint* x=copybigint(one,NULL);
...
{ vector* shape; entry* t=v->compon,* sh; _index i,d,n=v->ncomp, max=0;
...
{ _index i,r; _index* lambda,* skew;
  
  { _index c=n;  /* sufficiently large starting value */
    if (n==1) return false;
  
    lambda=alloc_array(_index,2*n+1); skew=&lambda[n];
...
{ _index N=1; entry* w=mkintarray(n); copyrow(v,w,n); sortrow(w,n);
  { _index i=0,j=n-1; while (i<j) swap(&w[i++],&w[j--]); }
    /* increasing order */
  
  { _index i=0, mult=1;
    while (++i<n) { N*=i+1; if (w[i]>w[i-1]) mult=1; else N /= ++mult; }
  }
  { matrix* result=mkmatrix(N,n); _index i=0;
...
{ matrix* result=mkmatrix(n_parts(n),n);
  if (n>0)
  { entry* lambda=mkintarray(n),** res=result->elm; _index i=0,j;
    lambda[0]=n; 
...
{ bigint* nt=n_tableaux(lambda,l); _index n=check_part(lambda,l);
  ...
  { _index i=0; do copyrow(t,res[i++],n); while(Nexttableau(t,n)); }
...
{ _index i,s=0; /* count non-zero even parts: */
...
#if 0
static entry Young_char_val(entry* lambda, entry* mu, _index l, _index m)
{ _index i=0; entry sum=0,li, M=m==0 ? 0 : mu[0]; /* largest part of |mu| */
  if (M==0) return 1; /* stop when |mu| is depleted */
  while (i<l && (li=lambda[i])>=M) /* try parts $\geq M$ of |lambda| */
  { _index j; entry c=1; /* number of equal parts */
...
#if 0
entry Schur_char_val(entry* lambda, entry* mu, _index l, _index m)
{ _index i; entry sum=0;
  ...
  { entry* lambda_prime=mkintarray(4*l)
    ...
    { copyrow(lambda_prime,nu,l); 
                                  { _index i; for (i=1; i<l; ++i) if (nu[i]>nu[i-1]) /* skip most cases */
                                    { entry nui=nu[i]; _index j=i;
...
#if 0
matrix* Schur_char(entry* lambda, _index l)
{ _index i,n=check_part(lambda,l); entry np=n_parts(n);
...
{ _index n=check_part(lambda,l);
  ...
  { 
    ...
    { i=0; /* _index of last entry that was set in~|mu| */
...
{ bigint* value=null; _index n=check_part(lambda,l),m2;
  if (n==0) return one;
