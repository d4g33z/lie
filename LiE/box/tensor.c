...
{ _index k,r=the_g->lierank; boolean neg;
...
{ _index r=the_g->lierank; boolean neg;
  addrow(lamrho,mu,cur_expon,r); /* |cur_expon=mu+lambda+rho| */
  neg=simp_make_dominant(cur_expon,the_g)%2!=0; /* apply |alt_dom| action */
...
{ poly* result; _index i,r=g->lierank;
...
{ if (type_of(grp)==SIMPGRP) return simp_tensor_irr(lambda,mu,nu,&grp->s);
  ...
  { poly* result;
    _index s=Ssrank(grp),td=grp->g.toraldim; /* size of toral part */
    ...
    { _index i;
      ...
      { simpgrp* g=Liecomp(grp,i); _index d=g->lierank;
...
{ _index i,j; poly* ans=poly_null(Lierank(grp));
  for (i=0; i<p->nrows; ++i)
    for (j=0; j<q->nrows; ++j)
