...
{ _index i,r=g->lierank; entry* res=mkintarray(r),* ones=mkintarray(r);
...
{ _index r=g->lierank;
  ...
  { entry* rho_hat=simp_level_vec(g); _index i,lev=0; 
    for (i=0; i<r; ++i) lev+=lambda[i]*rho_hat[i]; freearr(rho_hat);
    wt_levels=mkvector(lev/simp_detcart(g)+2);
  }
  
  { _index lev,d,k;
    _index lev_start=1,end=1; /* start and end of current level */
    ...
    for (lev=1; lev<wt_levels->ncomp-1; wt_levels->compon[++lev]=lev_start=end)
  	...
  	/* now root $\alpha_k$ has level |d| */
        ...
        { _index w;
            /* _index of a dominant weight $\mu\below\lambda$ at level |lev-d| */
...
local _index locate(entry* mu,entry** m,entry* rho_hat,entry* level,simpgrp* g)
{ _index l=0,i,r=g->lierank; entry* lambda=m[0]; /* the highest weight */
...
{ _index i,j,k,l,r=g->lierank,n=g->roots->nrows;
...
bigint* Dim(poly* p)
{ _index i,r=p->ncols; bigint* res=null; entry* lambda= mkintarray(r);
  for (i=0; i<p->nrows; ++i)
...
{ if (type_of(grp)==SIMPGRP) return simp_domw(lambda->compon,&grp->s,NULL);
  ...
  { _index i,j,n=grp->g.ncomp,r=Lierank(grp);
...
matrix* Cartes_prod(matrix** M,_index n,entry* toral,_index r)
{ _index i=0,j=0; entry* pos,* offset,* lambda; matrix* result;
                         ...
                         { _index i; entry* mu=dom_w->elm[k];
                           ...
                           for (i=0; i<roots->nrows; ++i)
                             ...
                             { entry* alpha=roots->elm[i];
                               ...
                               { _index k; entry* a=g->roots->elm[i]; entry inp=0,d=0;
...
{ _index i,j,r=Lierank(grp),s=Ssrank(grp); poly* result;
...
{ _index i,r=p->ncols; poly* result=poly_null(r);
  for (i=0; i<p->nrows; ++i)
  { testdom(p->elm[i],grp);
...
{ if (type_of(g)==SIMPGRP) return simp_level_vec(&g->s);
  ...
  { _index i,j=0,r=Lierank(g); entry* result=mkintarray(r);
