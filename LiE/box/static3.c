...
static void check_torals(matrix* m, _index r)
{ _index i;
  if (m->ncols!=r+1)
...
static void check_Ww(vector* ww, _index s)
{ _index i,l=ww->ncomp; entry* w=ww->compon;
  for (i=0; i<l; i++)
...
static void check_Wws(matrix* m, _index s)
{ _index i,j,l=m->ncols;
  for (i=0; i<m->nrows; i++)
  { entry* w=m->elm[i];
    for (j=0; j<l; j++) 
...
{ _index i, ng1=g1->g.ncomp, ng2=g2->g.ncomp;
...
static object groupmake(char lietype,_index rank)
{ object result;
  if (wronggroup(lietype,rank))
      error("Result %c%ld is illegal group.\n", lietype, (long)rank);
  if (lietype=='T')
  { result=(object) mkgroup((_index) 0); result->g.toraldim=rank; }
  else
  { result=(object) mkgroup((_index) 1);
...
{ _index s=Ssrank(grp=g); check_rt(v,s); check_rt(w,s); checkroot(w->compon);
...
{ _index r=Lierank(grp=g); matrix* m=mkmatrix(1,r+1); check_toral(t,r,0);
...
{ _index s=Ssrank(grp=g); check_rt(v,s); check_rt(w,s);
...
{ _index i,s=Ssrank(grp=g); vector* rw; entry* x=mkintarray(s); matrix* result;
...
{ _index s=Ssrank(grp=g); check_Ww(v,s); check_Ww(w,s);
...
{ _index i; matrix* result=(check_wts(m,Lierank(grp=g)),copymatrix(m));
  for (i=0; i<m->nrows; i++) make_dominant(result->elm[i]);
  return (object) result;
}

static object pol_dominant_pol_grp(p,g) poly* p; object g;
{ _index i; poly* result=(check_pol(p,Lierank(grp=g)),copypoly(p));
...
{ _index s=Ssrank(grp=g); poly* result; cmpfn_tp sav_cmp=cmpfn;
...
{ _index i,s=Ssrank(grp=g); entry* minus_rho=mkintarray(s); vector* result;
...
{ vector* result; _index l=ww->ncomp; entry* w=mkintarray(l);
...
{ vector* result; _index i,l=ww->ncomp; entry* w=mkintarray(l);
  check_Ww(ww,Ssrank(grp=g)); for (i=0; i<l; i++) w[i]=ww->compon[l-1-i];
  result=L_red(R,w,l); freearr(w);
  { _index n=result->ncomp-1; entry* res=result->compon; /* reverse */
...
{ _index s=Ssrank(grp=g); poly* result; check_Ww(x,s); check_Ww(y,s);
...
{ _index s=Ssrank(grp=g); vector* result; check_rt(v,s); check_Ww(word,s);
...
{ _index l=lambda->ncomp; check_part(lambda->compon,l);
...
{ _index l=v->ncomp; vector* result;
...
{ _index n=v->ncomp; vector* result;
...
{ _index l=t->ncomp; vector* result; entry* res;
...
{ _index n=P->ncomp; entry* p,* q; vector* w;
  if (n!=Q->ncomp) error("Tableaux not of same size");
  { _index i; vector* sp=check_tabl(P),* sq=check_tabl(Q);
...
{ _index i,n=W->ncomp; entry* w=W->compon; matrix* pq;
  for(i=0; i<n; i++)
...
{ entry* lambda=v->compon; _index l=v->ncomp;
...
{ entry* lambda=a->compon,* mu=b->compon;
  _index l=a->ncomp,m=b->ncomp,n=check_part(lambda,l);
...
{ _index r=Lierank(grp=g),n=d->intval;
...
{ _index r=Lierank(grp=g),n=d->i.intval;
...
{ _index R=Lierank(g),r=Lierank(grp=h); entry* lambda=v->compon;
  check_wt(v,R);
  if (m->m.nrows!=R)
...
{ _index R=Lierank(g),r=Lierank(grp=h);
  p=check_pol(p,R);
  if (m->m.nrows!=R)
    error ("Number of rows of restriction matrix should match Lie rank.\n");
  if (m->m.ncols!=r)
...
{ _index i,r=Lierank(grp=h),R=Lierank(g); entry d=n->intval; check_pol(p,r);
  for(i=0;i<p->nrows;i++) testdom(p->elm[i],h);
  if (m->nrows!=r) error ("Number of rows of inverse restriction matrix\
 should match rank subgroup.\n");
  if (m->ncols!=R) error ("Number of columns of inverse restriction matrix\
...
{ _index i,s=Ssrank(grp=g); entry* minus_rho=mkintarray(s); vector* w;
...
{ _index r=Lierank(grp=g); vector* t=(check_wt(v,r),Dominant(v));
...
{ _index r=Lierank(grp=g); entry* mu=mkintarray(r); copyrow(w->compon,mu,r);
...
{ _index i,r=Lierank(grp=g); entry* mu=mkintarray(r); bigint* result=null;
...
{ _index n=check_part(lambda->compon,lambda->ncomp);
  check_pol(p,Lierank(grp=g));
