...
{ _index i,n=p->nrows; poly* result=private_pol(p); bigint** b;
...
poly* Alt_dom(poly* p)
{ _index j,s=Ssrank(grp); /* keep out of torus */
  if (!s) return p; p=thorough_copy(p);
  for (j=0; j<p->nrows; j++)
  { register _index i=0,n=0; entry* v=p->elm[j]; bigint* c=p->coef[j];
    while(true)
...
static void simp_alt_refls(poly* p,_index offset, _index i,simpgrp* g)
{ bigint** c=p->coef; entry** m=p->elm;
  _index n=p->nrows,j,k=offset+i,r=g->lierank; entry d,* v; bigint* b;
  ...
  break; case 'D':
    ...
    else /* i==r-2 or i==r-1 */ { _index j=r-3-i; loop ( v[j]+=d )}
  break; case 'E':
...
static void alt_refls(poly* p,_index nr)
...
{ _index i; entry* w=word->compon,wi; p=thorough_copy(p);
...
static _index maindex;

static cmp_tp main_decr(entry* v, entry* w, _index len)
{ if (v[maindex]!=w[maindex]) return v[maindex]>w[maindex] ? 1 : -1;
  while (len-->0) if (*v++!= *w++) return *--v > *--w ? 1 : -1; return 0;
}

static poly* demaz(poly* p,_index nr)
{ _index i,k=0,pos,r=p->ncols,upb_size=0; poly* result;
