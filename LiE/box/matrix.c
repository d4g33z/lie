...
{ register _index j; _index i, k; entry sum,* cikptr; register entry* aijptr;
  for (i = 0; i<l; i++)
  { cikptr= *c++;
    for (k=0; k<n; k++)
    { sum=0; aijptr=a[i];
...
{ register _index i; _index j; entry sum; register entry* viptr;
  for (j=0; j<n; j++)
  { sum=0; viptr=v;
...
{ register _index j; _index i; entry sum; register entry* aijptr,* vjptr;
  for (i=0; i<m; i++)
...
{ _index i=a->nrows,j,k=a->ncols,l=b->ncols;
...
void printarr(a,n) entry* a; _index n;
{ Printf("[");
