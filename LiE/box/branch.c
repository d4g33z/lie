...
static _index r;   /* the lie rank of the (main) group */
...
{ poly* domchar=simp_domchar(lambda,NULL,g),* result=mkpoly(ord,1); _index i;
...
{ _index i,j,S=Ssrank(G),td=G->g.toraldim; poly* x; entry exp;
  lambda+=S; t+=S;
  x=mkpoly(1,1); *x->coef=one;
  exp=inprow(lambda,h,td)%ord; if (exp<0) exp+=ord; x->elm[0][0]=exp;
  for (i=G->g.ncomp-1; i>=0; i--)
  { simpgrp* g=Liecomp(G,i); _index d=g->lierank; lambda-=d; t-=d;
...
{ entry* t=toral_elt->compon,** lambda=p->elm; _index i,r=toral_elt->ncomp-1;
...
static _index rsub   /* the lie rank of the "sub" group */
...
{ register _index i; mulvecmatelm(v,resmat,add_wt,r,rsub); /* add_wt=v*m */
...
{ poly* domchar; _index i; r=g->lierank; resmat=m;
...
{ _index i,S=Ssrank(G),td=G->g.toraldim,r=Lierank(grp); poly* x;
  ...
  *x->coef=one; x=Alt_dom(x); /* ensure dominant */
  for (i=G->g.ncomp-1; i>=0; i--)
  { simpgrp* g=Liecomp(G,i); _index d=g->lierank; lambda-=d; m-=d;
...
{ _index i; entry** lambda=p->elm; poly* ans=poly_null(rsub=Lierank(grp));
...
{ _index i,j,r=Lierank(grp),s=Ssrank(g);
