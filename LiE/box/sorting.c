...
local void heapify_m(matrix* m,_index i,_index n)
{ _index l,len=m->ncols; entry** a=m->elm-1; /* array~$[1:n]$ */
  while ((l=i<<1)<=n)
  { _index d= (*compare)(a[i],a[l],len)<0 ? i : l;
...
local void heapify_p(poly* p,_index i,_index n)
{ _index l,len=p->ncols; entry** a=p->elm-1; bigint** coef=p->coef-1;
  while ((l=i<<1)<=n)
  { _index d= (*compare)(a[i],a[l],len)<0 ? i : l;
    if (l<n && (*compare)(a[d],a[l+1],len)>0) d=l+1;
      /* index of minimum among $\{i,l,l+1\}$ */
...
{ _index i,n=m->nrows; for (i=n; i>0; i--) heapify_m(m,i,n); }

local void build_heap_p(poly* p)
{ _index i,n=p->nrows; for (i=n; i>0; i--) heapify_p(p,i,n); }

local void heap_sort_m(matrix* m, cmpfn_tp criterion)
{ _index i=m->nrows; entry** a=m->elm; if (i<2) return;
...
{ _index i=p->nrows; entry** a=p->elm; bigint** coef=p->coef; if (i<2) return;
...
cmpfn_tp set_ordering (cmpfn_tp criterion, _index n, object g)
...
void swap_terms(entry** w,bigint** coef,_index i,_index j)
{ swap_rows(&w[i],&w[j]);
...
{ if (n>=3)
  { _index i=split_vec(a,n); sortrow(a,i); sortrow(&a[i+1],n-i-1); }
  else if (n==2 && a[0]<a[1]) swap(a,a+1);
}

local void sort_matrix (entry** m,_index n,_index c)
{ if (n>=3)
  { _index i=split_mat(m,n,c);
...
{ _index len=m->ncols; register entry** to=m->elm,** from=to,** end=to+m->nrows;
...
{ entry** expon=p->elm; bigint** coef=p->coef; _index t=0,f=0,len=p->ncols;
  heap_sort_p(p,cmpfn);
