...
local void fundam(matrix* roots, _index first, _index* last);


local _index s; /* the current semisimple rank */
...
{ matrix* adjs=mkmatrix(n,3);
  ...
  { _index e; /* index of end node (|valency[e]<=1|) */
...
static void cycle_block(matrix* m, _index first, _index last, _index amount)
{ entry** row=&m->elm[first];  _index modulus=last-first,i,min_done=amount;
  if (amount>0 && modulus>amount) /* otherwise there is nothing to do */
    for (i=0; i<min_done; ++i) /* |min_done| is fixed after first time round */
    { entry* row_i=row[i]; _index j=i+amount,old_j=i;
...
local void long_close(matrix* m, _index first, _index last)
{ _index i,j;  entry* root_i,* root_j,* t=mkintarray(s);
        ...
        { copyrow(root_j,t,s); add_xrow_to(t,-c,root_i,s);
          ...
          { j=i; c=Cart_inprod(root_i,root_j);
            ...
            if (isposroot(t)) copyrow(t,root_i,s);
                 ...
                 { _index k;  entry* ln,* sh; /* the longer and the shorter root */
...
{ _index i,j,k,r=Lierank(grp),s=Ssrank(grp), n=roots->nrows;
  ...
  { entry* v=img[j], norm=(checkroot(alpha[j]),Norm(alpha[j]));
    ...
    for (i=0; i<s; i++) 
                    { _index inpr= norms[i]*alpha[j][i]; /* this is $(\omega_i,\alpha[j])$ */
