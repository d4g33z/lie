...
{ _index i,r=the_g->lierank; boolean neg;
...
{ if (type_of(grp)==SIMPGRP) return simp_vdecomp_irr(lambda,&grp->s);
  ...
  { poly* result; _index i;
    
    { _index td=grp->g.toraldim;  lambda+=Ssrank(grp);
...
poly* Vdecomp(poly* p)
{ _index i,r=Lierank(grp); poly* result=poly_null(r);
