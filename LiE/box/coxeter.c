...
{ _index i,r=g->lierank; entry level=0,result; boolean neg;
...
local matrix* simp_refl(entry* alpha,simpgrp* g,matrix* m, _index offset)
{ _index i,r=g->lierank; entry* row=mkintarray(2*r),* col=&row[r];
  
  { _index norm_alpha=simp_norm(alpha,g); entry* norm=g->root_norm->compon;
...
{ _index i,j,t=0; matrix* R=mat_id(Lierank(grp));
...
_index simp_make_dominant(entry* lambda, simpgrp* g)
{ _index i=0,l=0, r=g->lierank;
  do
...
_index make_dominant(entry* lambda)
{ _index i=0,l=0; _index s=Ssrank(grp); /* do not affect the toral part */
...
local _index dom_length(entry* lambda)
{ _index l,s=Ssrank(grp); entry* x=mkintarray(s);  copyrow(lambda,x,s);
...
_index Length(vector* w)
{ _index i,s=Ssrank(grp),l=0; entry* x=mkintarray(s),wi;
...
vector* Wwordv(entry* lambda, _index l)
{ _index i=0,j=0,s=Ssrank(grp); entry* x=mkintarray(s);
...
finish: result=Wwordv(y,l); freearr(x);
  ...
  { matrix* check=Weyl_mat(result); _index r=Lierank(grp);
...
{ _index i,l=w->ncomp,s=Ssrank(grp),wi;
...
{ _index k,n=m->nrows,s=Ssrank(grp); matrix* result=mkmatrix(n,m->ncols);
  entry* x=mkintarray(s);
  for (k=0; k<n; ++k)
  { entry* w= m->elm[k]; _index i,l=m->ncols,wi; vector* t;
    for (i=0; i<s; ++i) x[i]=1;
    for (i=l-1; i>=0; --i)
...
{ _index i,j,s=Ssrank(grp),count=0; entry* inx=mkintarray(m->nrows);
  for (i=0; i<m->nrows; ++i) /* get indices of dominant rows into |inx| */
  { for (j=0; j<s; ++j)  if (m->elm[i][j]<0) break;
    if (j==s) inx[count++]=i;
  }
  { _index r=m->ncols; matrix* result=mkmatrix(count,r);
...
{ _index i,j,s=Ssrank(grp),count=0; entry* inx=mkintarray(p->nrows);
  ...
  { _index r=p->ncols; poly* result=mkpoly(count,r);
    for (i=0; i<count; ++i)
...
local entry* fix_vec (vector* I, _index s)
{ _index i; entry* x=mkintarray(s);
...
vector* L_red(vector* L, entry* w, _index lw)
{ _index i,s=Ssrank(grp),l=lw;
  ...
  for (i=0; i<lw; i++)
    ...
    { entry wi=w[i]-1; /* internally, reflections are numbered $0,\ldots,s-1$ */
      if (w[i]==0) --l; /* ignore zero entry */
        ...
        { _index j=i;  copyrow(x,y,s); /* make a temporary copy of~|x| */
          do
            if (w[--j]!=0)
...
{ _index i,s=Ssrank(grp), d; entry* delta=mkintarray(s);
  
  { matrix* ic=Icartan(); _index dc=Detcartan(); entry* t=mkintarray(s);
...
{ _index i,s=Ssrank(grp); entry* x=mkintarray(2*s),* y=&x[s]; boolean result;
...
matrix* Bh_desc(entry* w, _index l) /* |w| is reduced of length |l| */
{ entry* inx; _index s=Ssrank(grp),n_desc=0;
  if (l==0) return mkmatrix(0,0); /* avoid attempting a $0\times-1$ matrix */
  
  { _index i,j; entry* x=(inx=mkintarray(l+s),&inx[l]);
    for (i=0; i<l; ++i)
  ...
  { matrix* result=mkmatrix(n_desc,l-1); _index i,j;
    for (i=0; i<n_desc; ++i)
...
matrix* Bh_desc_rel(entry* w, _index l, entry* lwb)
{ entry* inx; _index s=Ssrank(grp),n_desc=0;
  if (l==0) return mkmatrix(0,0); /* avoid attempting a $0\times-1$ matrix */
  
  { _index i,j; entry* x=(inx=mkintarray(l+2*s),&inx[l]), *y=&x[s];
    for (i=0; i<l; ++i)
    { for (j=0; j<s; ++j) x[j]=1; /* |x=rho| */
      for (j=0; j<l; ++j)  if (j!=i)
      { entry wj=w[j]-1; if (x[wj]>0) w_refl(x,wj); else break; }
...
local void Bh_descs(matrix** m, entry* lwb, _index s)
{ vector* word=Wwordv((*m)->elm[0],-1); entry* w=word->compon;
  _index i,j,k,l=word->ncomp,n_desc=0;
...
local poly* R(entry* x, entry* y, _index k, _index d, _index s)
  /* modifies |x| and |y| */
{ _index i; entry* v=mkintarray(2*s),* w=&v[s];
...
{ _index i,s=Ssrank(grp); entry* v=mkintarray(2*s),* w=&v[s]; poly* result;
  for (i=0; i<s; ++i) v[i]=w[i]=1;
  Waction(v,x); Waction(w,y);
  result=R(v,w,0,dom_length(w)-dom_length(v),s); freearr(v); return result;
}

local poly* KL(entry* x, entry* y, _index e, _index d, _index s)
{ _index i; entry* v=mkintarray(2*s),* w=&v[s]; poly* result;
  ...
  { matrix* level=mkmatrix(1,s); _index j,c=1; /* |c==(l(y)-l(z)+1)/2| */
