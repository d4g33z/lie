...
static object matmake(symblst arglst)
{
  object result;
  symblst list;
  _index i, j, nr=0, nc=0, nc_last=0;
...
static object vec_apply(symblst funsym)
{
  ...
  if (vec2sym) {
    ...
    if (vec2sym->data.val->v.ncomp != size)
      error("_index vector arguments unequal.\n");
  }
  result = (object) mkvector(size);
  addstaynode(result);
  f= newnode(funsym->a.label);
    ...
    object	    a, b;
{
    ...
    entry g = b->i.intval;
    object	    result;
    _index n = a->m.ncols, m =a->m.nrows;
    if (!g)
    ...
    object	    a, b;
{
    _index  i, j;
    entry g = b->i.intval;
    _index m = a->m.nrows, n = a->m.ncols;
    object	    result;
    if (g<0)
	error("LiE can only take the modulus by a positive number.\n");
    result = (object) mkmatrix(a->m.nrows, a->m.ncols);
...
vec_mul_vec_mat(v, m)
    object	    v, m;
{
    _index    i, k, nrows=m->m.nrows, ncols=m->m.ncols;
    object	    result;
    if (v->v.ncomp != nrows)
...
{
    object result;
    _index i,nv=v->v.ncomp, nw=w->v.ncomp;
...
matrix* mat_null (_index r,_index c)
{ _index i,j; matrix* m=mkmatrix(r,c); entry** me=m->elm;
  for (i=0;i<r;++i) for (j=0;j<c;++j) me[i][j]=0;
  return m;
}

matrix* mat_id(_index size)
{ _index i,j; matrix* m=mkmatrix(size,size); entry** me=m->elm;
...
{
    object result;
    _index i,n1=a->m.nrows,n2=b->m.nrows,m;
...
{
  _index i; vector* a=&v->v,* b=&w->v; vector* result;
    ...
    object	    a, b;
{
    ...
    _index    i,j;
    entry g = a->i.intval;
    _index n = b->m.ncols, m = b->m.nrows;
...
{ _index r = Integer(m_obj), c = Integer(n_obj);
...
static object mat_one_int_int(m_obj,n_obj)
    object m_obj,n_obj;
{ _index i,j,r=Integer(m_obj), c=Integer(n_obj); matrix* m;
  if (r<0) error("row size<0\n");
  if (c<0) error("column size<0\n");
  m=mkmatrix(r,c);
...
{ _index n=nn->intval, i=ii->intval,j; vector* result; entry* v;
  if (i<0 || i>n)
...
{ _index n=nn->intval,j; vector* result; entry* v;
  if (n<0)
