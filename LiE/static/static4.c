...
{
    _index nrows = a->nrows;
    _index nvars = a->ncols;
    _index index1;
    poly	  *result;
    if (nvars != v->ncomp) 
...
{ _index nrows=a->nrows, ncols=a->ncols, ncomp=e->ncomp,kval=Integer(n)-1;
  matrix* result;
  if (kval < 0 || kval >= nrows)
      error("Row index (%ld) into matrix out of range. \n",(long)(kval+1));
  if (ncomp != ncols)
...
{ size_t l=1+v->ncomp; _index i; char buf[20];
  tekst *result; char* p;
  if (v->ncomp==0) l=2;
  else
...
{
    object	    result;
    _index  m = a->pl.nrows, n = a->pl.ncols, mp = p->pl.nrows;
    _index	      i,j,k; 
    bigint *coef;
