...
{
  _index size=v->v.ncomp, nrows, ncols=nrows_object->i.intval;
  if (ncols<=0) error("Number of columns should be positive.\n");
  if (size%ncols!=0) 
    error ("Number of columns should divide size of vector.\n");
  { matrix* m=mkmatrix(nrows=size/ncols,ncols); _index i,j,k=0;
    for (i=0; i<nrows; ++i)
      for (j=0; j<ncols; ++j)
...
{
    _index i; boolean is_zero=true;
    _index n = v_vec->ncomp;
