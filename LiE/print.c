...
local entry* max_entry_widths(entry** m, _index nrows, _index ncols)
{ char s[entry_width+1]; _index i,j;
...
local int print_row(_index n, entry* v, entry* width, _index start_col)
{ _index i; boolean wrapped=false;
...
{ if (m->nrows==0) { indent(lmargin); Printf("null(0,%ld)",(long)m->ncols); }
  ...
  { _index i,n=m->nrows; entry* widths=max_entry_widths(m->elm,n,m->ncols);
